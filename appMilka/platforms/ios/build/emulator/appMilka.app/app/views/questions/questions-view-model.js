...
function createViewModel(database) {
    ...
    this.QuestionsAgree = new ObservableArray ([]);
      database.all("SELECT question, count(A) from questions").then(rows => {
        for(var row in rows) {
        this.QuestionsAgree.push({question: rows[row][0], agree: rows[row][1]});
          console.log(this.QuestionsAgree);
        }
          }, error => {
          console.log("SELECT ERROR", error);
      });
